...
scale_height <- function() {
    if (length(input$VariableSelect) <= 2) {
        ...
        length(input$VariableSelect) > 2 & length(input$VariableSelect) <= 4
    ) {
        800
    } else if (
        length(input$VariableSelect) > 4 & length(input$VariableSelect) <= 6
    ) {
        900
    } else if (
        length(input$VariableSelect) > 6 & length(input$VariableSelect) <= 8
    ) {
        1000
    } else if (
        length(input$VariableSelect) > 8 & length(input$VariableSelect) <= 10
    ) {
        1100
    } else {
        1200
...
output$PlotMain <- renderPlot(
    {
        validate(need(
            !(sum(!is.na(as.numeric(DatSub()$VALUE))) == 0 &
                metricstatselections()$stat == 'Total'),
            paste(
...
output$dlTable <- downloadHandler(
    ...
    content = function(file) {
        ...
        names(table)[
            names(table) == 'source'
        ] <- "Sourced from the FISHEyE application (http://dataexplorer.northwestscience.fisheries.noaa.gov/fisheye/PerformanceMetrics/) maintained by NOAA Fisheriess NWFSC. Technical information can be found here: https://repository.library.noaa.gov/view/noaa/31435"
    ...
    content = function(file) {
        ...
        if (length(input$VariableSelect) <= 6) {
            ...
            dat <- subset(
                dat,
                VARIABLE %in%
                    input$VariableSelect[7:length(input$VariableSelect)]
            )
            doPlotDownload(dat = dat, x = "YEAR", y = "VALUE")
        }
        dev.off()
        ...
        wellPanel(
            ...
            if (lvls[round(length(lvls) * input$plot_hover$x)] == 2015) {
                tags$div(tags$p(
                    'The Blob and El',
                    HTML("Ni&ntilde;o"),
                    'converge. Biomass is low.',
                    tags$br(),
                    'Council establishes catch limits for unfished and unmanaged forage fish.',
                    tags$br(),
                    'Canary rockfish and petrale sole are declared rebult.',
                    tags$br(),
                    'Dover sole and thornyhead limits increased.'
                ))
            } else if (lvls[round(length(lvls) * input$plot_hover$x)] == 2014) {
                tags$div(
                    'Fishery participants can buy and sell quota shares.',
                    tags$br(),
                    'Non-whiting groundfish gets MSC certification',
                    tags$br(),
                ...
                tags$div(
                    HTML("T&#333;hoku earthquake and tsunami hits Japan."),
                    tags$br(),
                    "Sablefish prices are high.",
                    tags$br(),
                ...
                tags$div(
                    'Sector-specific quota allocation for bycatch species.',
                    tags$br(),
    ...
    if (!is.null(input$plot_click)) {
        ...
        wellPanel(
            if (lvls[round(length(lvls) * input$plot_click$x)] == 2015) {
                tags$div(tags$p(
                    'The Blob (first detected in 2013) and El',
                    HTML("Ni&ntilde;o"),
                    'converge, leading to record low biomass of key prey species for many catch share species; catch attainment is lower for Pacific whiting.',
                    tags$a(
                        href = 'http://www.nationalgeographic.com/magazine/2016/09/warm-water-pacific-coast-algae-nino/',
                ...
                tags$div(
                    ...
                    tags$a(
                        href = 'https://www.nytimes.com/2014/08/08/world/europe/russia-sanctions.html',
                        'Read this NYTimes article for background.',
                        target = '_blank'
                    ),
                    'The ban is still in place.
                 According to a',
                    tags$a(
                        href = 'http://trade.ec.europa.eu/doclib/docs/2015/december/tradoc_154025.pdf',
                        'paper in Chief Economist Note,',
                        target = '_blank'
                    ),
                    'the impact of the ban on the US fisheries industry has been minimal.',
                    tags$br(),
                ...
                paste0(
                    'Widow rockfish, a species that was constraining co-occurring target species, is declared rebuilt.'
                )
                ...
                paste0(
                    'Sector-specific bycatch species quota allocation ends race-to-fish for constraining species between at-sea motherships.
               Pacific whiting gets MSC certification.'
                )
            }
        )
